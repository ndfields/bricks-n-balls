...
**Difficulty evens out at high levels.** Because you always get the same 60 balls, your damage per turn is roughly constant, so brick toughness can't rise forever or the game becomes unwinnable. It climbs normally through about level 14, then approaches a hard ceiling it never crosses (`CEIL` in `levelBase`), so a row at level 200 costs about the same as one at level 30 — the numbers genuinely plateau instead of creeping back up over hundreds of levels. Progression past that point comes from the full row count and denser, heavier boards, not ever-bigger numbers.
